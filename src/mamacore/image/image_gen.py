...
class GeminiProvider(ImageProvider):
    ...
    def generate(self, prompt: str, size: str) -> bytes:
        ...
        # Try standard Gemini inlineData format
        for part in parts:
            inline = part.get("inlineData")
            if inline and inline.get("mimeType", "").startswith("image/"):
                return base64.b64decode(inline["data"])

        # Try miaoda markdown format: ![image](data:image/jpeg;base64,...)
        for part in parts:
            text = part.get("text", "")
...
PROVIDERS = {
    ...
    "miaoda": GeminiProvider,  # miaoda uses Gemini-compatible endpoint
}
