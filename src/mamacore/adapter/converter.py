...
from mamacore.adapter.theme_lib import Theme, load_theme, get_inline_css_rules
...
class WeChatConverter:
    ...
    def _apply_wechat_fixes(self, html: str) -> str:
        ...
        2. Convert code block newlines to <br> for reliable WeChat rendering
        3. Add language label to code blocks
        """
        soup = BeautifulSoup(html, "html.parser")
        ...
        # Fix 2 & 3: Convert <pre> code blocks to <section> with <br> line breaks
        for pre in soup.find_all("pre"):
            code = pre.find("code")
            ...
            # Detect language from code class or data-lang
            lang = None
            if code:
                for cls in code.get("class", []):
                    if cls.startswith("language-"):
            ...
            # Check if wrapped in codehilite container
            container = pre.find_parent("div", class_="codehilite") or pre

            # Build each line as <span> + <br>
            lines = raw.split("\n")
            ...
            for line in lines:
                ...
                if safe:
                    inner_parts.append(f'<span>{safe}</span>')
                else:
                    inner_parts.append('<span>&nbsp;</span>')

            br_joined = "<br>".join(inner_parts)

            # Language label div
            label_html = ""
            if lang:
                label_html = (
                    '<div style="font-size:12px;color:#636d83;'
                    'font-family:SFMono-Regular,Consolas,monospace;'
                    'margin-bottom:4px;letter-spacing:0.5px">' + lang + '</div>'
                )

            # Build new container: dark background + monospace
            base_style = (
            ...
            new_div = soup.new_tag("section", style=base_style)
            new_div.append(BeautifulSoup(label_html + br_joined, "html.parser"))
    ...
    def _inject_darkmode(self, html: str) -> str:
        ...
        # Headings - use primary color for visibility in dark mode
        dm_heading = darkmode.get("primary", darkmode.get("text", "#FF8C5A"))
    ...
    def _process_callout(self, text: str) -> str:
        ...
        def replace_callout(match):
            ...
            # Convert inline Markdown: **bold** -> <strong>, *italic* -> <em>
