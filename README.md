...
| 组件 | 内容 | 安装方式 |
|------|------|----------|
| **MCP Server** | 21 个 MCP Tools | `claude mcp add` 或 `.mcp.json` |
| **Skills** | 3 个 SKILL.md 工作流 | `npx skills add` |
| **CLI** | 8 个 `mama` 命令 | `pip install` |
...
# 3. 注册 MCP Server（21 个 Tools）
claude mcp add mamacore -- mama-server
```

安装后在**任何目录**打开 Claude Code，即可使用：
- **21 个 MCP Tools**：Claude Code 自动按需调用
- **3 个 Skill**：输入 `/wechat-article` 等命令触发
- **8 个 CLI 命令**：终端直接运行 `mama`

### 方式二：项目级使用（无需安装）

```bash
git clone https://github.com/yqdaddy/mamacore.git
cd mamacore && uv sync && claude
```
...
安装后 `mama` 和 `mama-server` 命令全局可用。

### 方式四：开发模式

```bash
git clone https://github.com/yqdaddy/mamacore.git
cd mamacore && pip install -e .
```
