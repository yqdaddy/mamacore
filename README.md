...
M.A.M.A. Core 是通用的 MCP Server 和 Skills，支持 **Claude Code、OpenClaw、Hermes、Codex** 等所有兼容 MCP 的智能体。

### 快速安装（所有 Agent 通用）

只需两步，即可完成 **CLI 工具** 和 **Skills 工作流** 的全局安装：

```bash
# 1. 安装 CLI（8 个命令）和 MCP Server
pip install git+https://github.com/yqdaddy/mamacore.git

# 2. 安装 Skills（3 个工作流，所有 Agent 通用）
npx skills add yqdaddy/mamacore --global
```

### Agent 专属注册（让智能体发现 MCP Server）

根据你的主力智能体，运行以下命令将 MCP Server 注册到配置中：
...
#### OpenClaw
```bash
openclaw mcp add mamacore -- mama-server
```

#### Hermes / Codex
通常会自动检测当前目录下的 `.mcp.json`。如果你希望全局使用，请在对应 Agent 的配置文件中添加：
...
此时在该目录下启动智能体（如 `claude` 或 `openclaw`），会自动加载 MCP Server 和 Skills。

### 初始化配置

```bash
cp .env.example .env               # 编辑填入 API Key
cd services/dailyhot && npm install  # 热榜服务依赖（仅需一次）
```

热榜服务在首次调用时自动启动，无需手动干预：

```bash
mama hot-today -s weibo -n 20   # 自动启动 → 查询 → 返回结果
```
...
在 Claude Code / OpenClaw / Hermes 中使用：
