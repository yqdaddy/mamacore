...
# （可选）安装 DailyHotApi 热榜服务
cd services/dailyhot && pnpm install && cd ../..
```
...
| 变量                       | 说明                         | 必填             |
| ------------------------ | -------------------------- | -------------- |
| `OPENAI_API_KEY`         | DALL-E 3 图片生成              | 否              |
| `DASHSCOPE_API_KEY`      | 通义万相图片生成                   | 否              |
...
| `DAILYHOT_API_URL`       | DailyHotApi 实例地址（已弃用，改用内置 API）    | 否              |
